...
class CrudCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		foreach($files as $file)
		{
			if(is_file($templatePath.'/'.$file) &&
               (CFileHelper::getExtension($file)==='tpl' || CFileHelper::getExtension($file)==='php')
               && $file!=='controller.php')
			{
				$this->files[]=new CCodeFile(
	...
	public function generateInputField($modelClass,$column)
	{
		if($column->type==='boolean')
			...
			return "CHtml::activeTextArea(\$model,'{$column->name}',['rows'=>6, 'cols'=>50])";
		else
		{
			...
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "CHtml::{$inputField}(\$model,'{$column->name}',['size'=>$size,'maxlength'=>$maxLength])";
			}
		}
	}
	...
	public function generateActiveField($modelClass,$column)
	{
		if($column->type==='boolean')
			...
			return "\$form->textArea(\$model,'{$column->name}',['rows'=>6, 'cols'=>50])";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',['size'=>$size,'maxlength'=>$maxLength])";
			}
		}
	}
